...
class BiomartGeneAnnotationsReader(object):
    ...
    def _get_genes(self):
        genes = self.dataset_grch37.query(
            attributes=[
                "ensembl_gene_id",
        ...
        genes["HGNC symbol"].fillna(genes["Gene stable ID"], inplace=True)
        # removes thes annoying [Source:HGNC Symbol;Acc:29523] from gene description
        genes["Gene description"] = genes["Gene description"].transform(
            lambda x: re.sub(r"\[.*\]", "", x).strip() if isinstance(x, str) else x
        )
        return genes

    def _get_gene_symbols_grch38(self):
        return self.dataset_grch38.query(attributes=["ensembl_gene_id", "hgnc_symbol"], filters=self.filters)
    ...
    def _merge_synonyms(synonyms_grch37, synonyms_grch38):
        return list(
            set(
                BiomartGeneAnnotationsReader._filter_empty_values_from_list(synonyms_grch37)
    ...
    def load_data(self):
        ...
        gene_synonyms = (
            gene_synonyms_grch37.reset_index()
            .join(gene_synonyms_grch38, on=["Gene stable ID"], rsuffix="_grch38")
            .fillna("")
        )
        ...
        data = (
            genes.join(canonical_transcripts.set_index("Gene stable ID"), on=["Gene stable ID"])
            .join(gene_synonyms_grch37, on=["Gene stable ID"])
            .drop_duplicates(subset="Gene stable ID")
            .join(gene_symbols_grch38.set_index("Gene stable ID"), on=["Gene stable ID"], rsuffix="_grch38")
        )

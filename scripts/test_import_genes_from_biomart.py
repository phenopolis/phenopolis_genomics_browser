...
from .import_genes_from_biomart import BiomartGeneAnnotationsReader
...
class TestBiomartGeneAnnotationsReader(TestCase):
    ...
    def test_loading(self):
        filters = {
            'chromosome_name': '22',
            'transcript_biotype': [
                'protein_coding', 'IG_C_gene', 'IG_D_gene', 'IG_J_gene', 'IG_V_gene', 'TR_C_gene', 'TR_D_gene',
                'TR_J_gene', 'TR_V_gene'
            ],
        ...
        self.assertEqual(data['HGNC symbol'].isnull().sum(), 0)     # no empty gene names
        ...
        self.assertEqual(phenopolis_data.gene_name.isnull().sum(), 0)

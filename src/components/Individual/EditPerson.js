...
class EditPerson extends React.Component {
  constructor(props) {
    ...
    this.state = {
      genderValue:
        this.props.metadata.data[0].sex === 'M'
          ? 'male'
          : this.props.metadata.data[0].sex === 'F'
          ? 'female'
          : 'unknown',
      consanguinityValue: this.props.metadata.data[0].consanguinity,
      featureArray: this.props.metadata.data[0].simplified_observed_features.map((x) => {
        return x.display;
      }),
      geneArray: this.props.metadata.data[0].genes.map((x) => {
        return x.display;
      }),
      ConfirmOpen: false,
    };
  ...
  handleSave = () => {
    ...
    axios
      .post('/api/update_patient_data/' + this.props.patientName, formData, {
...
export default compose(withStyles(dialogStyles), connect(null, { setSnack }))(EditPerson);

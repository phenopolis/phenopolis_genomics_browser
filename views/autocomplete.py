...
def _search_patients(cursor, query):
    ...
    '''
    cursor.execute(r''' select i.external_id, i.internal_id from individuals i, users_individuals ui where
...
def _search_genes(cursor, query):
    ...
    '''
    cursor.execute(r'''
                   select * from genes where gene_name_upper like %(suffix_query)s or other_names like %(query)s or gene_id like %(query)s limit %(limit)s''',
                   {'suffix_query': "%{}%".format(query.upper()), 'query': "%{}%".format(query.upper()), 'limit': SEARCH_RESULTS_LIMIT})
...
def _search_variants(cursor, query):
    ...
    if chrom is not None and ref is not None and alt is not None:
        cursor.execute(r'''select "CHROM", "POS", "REF", "ALT" from variants where
    ...
    # while the search is performed on the upper cased gene name, it returns the original gene name
#     return ["{chrom}-{pos}-{ref}-{alt}".format(chrom=x['CHROM'], pos=x['POS'], ref=x['REF'], alt=x['ALT']) for x in variant_hits]

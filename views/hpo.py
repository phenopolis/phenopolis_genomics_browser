...
def hpo(hpo_id='HP:0000001', subset='all', language='en'):
    ...
    parent_phenotypes = [{'display': i, 'end_href': j} for i, j, in
                         zip(res['hpo_ancestor_names'].split(';'), res['hpo_ancestor_ids'].split(';'))]
    ...
    if hpo_id != 'HP:0000001':
        ...
        config[0]['phenogenon_recessive']['data'] = [
            {'gene_id': [{'display': gene_id, 'end_href': gene_id}], 'hpo_id': hpo_id, 'hgf_score': hgf,
             'moi_score': moi_score} for gene_id, hpo_id, hgf, moi_score, in c.fetchall()]
        c.execute("select * from phenogenon where hpo_id='%s'" % hpo_id)
        ...
        config[0]['skat']['data'] = [
            {'gene_id': [{'display': gene_id, 'end_href': gene_id}], 'fisher_p_value': fisher_p_value, 'skato': skato,
             'odds_ratio': odds_ratio, 'variants': []} for gene_id, fisher_p_value, skato, odds_ratio, variants, in
            c.fetchall()[:100]]
    application.logger.debug(len(individuals))
    config[0]["preview"] = [["Number of Individuals", len(individuals)]]
    if subset == 'preview': return json.dumps([{subset: y['preview']} for y in config])
    for ind in individuals:
        ind['internal_id'] = [{'display': ind['internal_id']}]
        ind['simplified_observed_features_names'] = [{'display': i, 'end_href': j} for i, j, in
                                                     zip(ind['simplified_observed_features_names'].split(';'),
